...
function getFederationStats(stats, federationPlugin) {
    ...
    /** @type {string} */
    const remote = (federationPlugin._options.library?.name) ||
...
class FederationStatsPlugin {
     ...
     */
    apply(compiler) {
        const federationPlugins = compiler.options.plugins?.filter((plugin) => ['NextFederationPlugin', 'UniversalFederationPlugin', 'NodeFederationPlugin', 'ModuleFederationPlugin'].includes(plugin.constructor.name) && plugin?._options?.exposes);
